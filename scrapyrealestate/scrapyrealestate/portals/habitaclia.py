...
class HabitacliaAdapter(BasePortalAdapter):
    ...
    def _transaction_type(self, raw_url: str) -> TransactionType | None:
        ...
        if prefix in {"venta", "viviendas", "pisos", "casas", "aticos"}:
    ...
    def _build_search_url(self, search: NormalizedSearch, location_slug: str) -> str:
        # Current Habitaclia uses `viviendas-<municipio>` for sale and
        # `alquiler-<municipio>` for rent.
        segment = (
            "viviendas"
            if search.transaction_type is TransactionType.BUY
            else "alquiler"
        )
